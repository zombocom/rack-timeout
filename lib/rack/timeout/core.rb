...
module Rack
  class Timeout
    ...
    ENV_SERVICE_TIMEOUT_KEY = "rack-timeout.service_timeout".freeze # key under which each request may specify an override value for service_timeout.
    ...
    def call(env)
      ...
      service_timeout_env = env.fetch(ENV_SERVICE_TIMEOUT_KEY, service_timeout)

      # pass request through if service_timeout is false (i.e., don't time it out at all.)
      return @app.call(env) unless service_timeout_env
      ...
      info.timeout = service_timeout_env # nice and simple, when service_past_wait is true, not so much otherwise:
      info.timeout = seconds_service_left if !service_past_wait && seconds_service_left && seconds_service_left > 0 && seconds_service_left < service_timeout_env
